  ...
  return (
    <div className="p-8 space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
         ...
         <div className="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm">
            <span className="text-[10px] font-black text-black uppercase tracking-widest">Recovered This Month</span>
            <h3 className="text-3xl font-black text-emerald-600 tracking-tighter mt-2">₹0.00</h3>
            <div className="mt-4 flex items-center gap-2 text-emerald-600 font-black text-xs bg-emerald-50 w-fit px-3 py-1.5 rounded-full uppercase tracking-tight">
               <CheckCircle2 className="w-3.5 h-3.5" /> 0% from last month
            </div>
         </div>
         <div className="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm">
            <span className="text-[10px] font-black text-black uppercase tracking-widest">Average Settlement</span>
            <h3 className="text-3xl font-black text-indigo-600 tracking-tighter mt-2">0 Days</h3>
            <div className="mt-4 flex items-center gap-2 text-indigo-600 font-black text-xs bg-indigo-50 w-fit px-3 py-1.5 rounded-full uppercase tracking-tight">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50/50">
                <th className="px-8 py-6 text-[10px] font-black text-black uppercase tracking-[0.2em]">Transaction ID</th>
                <th className="px-8 py-6 text-[10px] font-black text-black uppercase tracking-[0.2em]">Institution</th>
                <th className="px-8 py-6 text-[10px] font-black text-black uppercase tracking-[0.2em]">Amount</th>
                <th className="px-8 py-6 text-[10px] font-black text-black uppercase tracking-[0.2em]">Status</th>
                <th className="px-8 py-6 text-[10px] font-black text-black uppercase tracking-[0.2em]">Date</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-50">
              {loading ? (
                <tr>
                   <td colSpan="5" className="px-8 py-12 text-center text-black font-black uppercase tracking-widest text-[11px]">Consolidating accounts...</td>
                </tr>
              ) : payments.length === 0 ? (
                <tr>
                   <td colSpan="5" className="px-8 py-12 text-center text-black font-black uppercase tracking-widest text-[11px]">No payment history available.</td>
                </tr>
              ) : (
                payments.map((p) => (
                  <tr key={p.id} className="hover:bg-gray-50/50 transition-colors group">
                    <td className="px-8 py-6 font-black text-black opacity-30 text-xs">#{p.id.slice(-8).toUpperCase()}</td>
