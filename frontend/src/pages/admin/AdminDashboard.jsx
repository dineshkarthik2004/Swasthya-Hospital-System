  ...
  return (
    <div className="p-8 space-y-8">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-gray-800 font-bold uppercase tracking-widest text-[11px] opacity-40">Real-time performance metrics across the network.</p>
      </div>
        ...
        {cards.map((card, i) => (
          <div key={i} className="bg-white p-6 rounded-[2.5rem] border border-gray-100 shadow-sm hover:shadow-xl transition-all duration-500 group">
            ...
            <div className="space-y-1">
              <p className="text-gray-800 text-[10px] font-bold uppercase tracking-[0.2em] opacity-40">{card.name}</p>
              <h3 className="text-3xl font-bold text-gray-900 tracking-tighter">{loading ? "..." : card.value}</h3>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-white rounded-[3rem] border border-gray-100 p-8 shadow-sm">
           <div className="flex justify-between items-center mb-8">
              <h3 className="text-xl font-black text-gray-900 tracking-tight text-black">Network Activity</h3>
              <div className="flex gap-2">
                 <div className="flex items-center gap-2 px-4 py-2 bg-gray-50 rounded-2xl border border-gray-100 cursor-pointer">
                    ...
                    <span className="text-[10px] font-black uppercase text-black tracking-widest">Hospitals</span>
                 </div>
                 <div className="flex items-center gap-2 px-4 py-2 bg-gray-50 rounded-2xl border border-gray-100 cursor-pointer">
                    <div className="w-2 h-2 bg-indigo-500 rounded-full"></div>
                    <span className="text-[10px] font-black uppercase text-black tracking-widest">Consultations</span>
                 </div>
              </div>
           </div>
           <div className="h-64 flex items-center justify-center bg-gray-50 rounded-[2rem] border-2 border-dashed border-gray-200">
              <div className="flex flex-col items-center gap-4">
                 <Activity className="w-10 h-10 text-black opacity-20 animate-pulse" />
                 <p className="text-[10px] font-black uppercase tracking-widest text-black">Activity Graph Visualization</p>
              </div>
           </div>
        </div>
